...
export const deletePluginDataApi = (items: PluginDeleteItem[]): Promise<IResponse<commonRespData>> => {
    return request.post({
...
export const cleanPluginLogApi = (module: string, hash: string): Promise<IResponse<LogRespData>> => {
    return request.post({ url: "/api/plugin/log/clean", data: { module, hash } });
};

export const getPluginDataByModuleApi = (module: string): Promise<IResponse<pluginDataResponse>> => {

...
export const upgradePortDictDataApi = (id: string, name: string, value: string): Promise<IResponse<dictRespData>> => {
    return request.post({ url: "/api/dictionary/port/upgrade", data: { id, name, value } });
};
export const addPortDictDataApi = (name: string, value: String): Promise<IResponse<dictRespData>> => {
